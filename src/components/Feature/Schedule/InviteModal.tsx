        ...
        setAllUsers((prevUsers) =>
          prevUsers.map((user) =>
            user.attendeeId === attendeeId
              ? { ...user, permission: newPermission as 'ALL' | 'EDIT' | 'CHAT' | 'READ' }
              : user
          )
        );
        setDropdownStates({});
                ...
                <DropdownMenu>
                  {permissions.slice(0, 4).map((permission) => (
                    ...
                    >
                      ...
                      <DropdownDescription>{permission.description}</DropdownDescription>
                      {selectedPermission === permission.value && <CheckMark>✔</CheckMark>}
                    </DropdownItem>
                  ))}
                </DropdownMenu>
              )}
                          ...
                          {loggedInIsAuthor ? (
                            <>
                              {permissions.slice(0, 4).map((permission) => (
                                <DropdownItem
                                  key={permission.value}
                                  onClick={() => handlePermissionChange(user.attendeeId, permission.value)}
                                >
                                  <strong>{permission.label}</strong>
                                  <DropdownDescription>{permission.description}</DropdownDescription>
                                  {user.permission === permission.value && <CheckMark>✔</CheckMark>}
                                </DropdownItem>
                              ))}
                              <hr />
                            ...
                            <>
                              <DropdownItem
                                key="CHAT"
                                onClick={() => handlePermissionChange(user.attendeeId, 'CHAT')}
                              >
                                <strong>{permissions.find(p => p.value === 'CHAT')?.label}</strong>
                                <DropdownDescription>{permissions.find(p => p.value === 'CHAT')?.description}</DropdownDescription>
                                {user.permission === 'CHAT' && <CheckMark>✔</CheckMark>}
                              </DropdownItem>
                              <hr />
                              <DropdownItem
                                key="LEAVE"
                                onClick={() => handlePermissionChange(user.attendeeId, 'LEAVE')}
                              >
                                <strong>{permissions.find(p => p.value === 'LEAVE')?.label}</strong>
                                <DropdownDescription>{permissions.find(p => p.value === 'LEAVE')?.description}</DropdownDescription>
                              </DropdownItem>
                            </>
                          )}
                        </DropdownMenu>
