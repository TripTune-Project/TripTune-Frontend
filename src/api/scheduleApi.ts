...
export const getSchedule = async (scheduleId: number) => {
  try {
    const response = await authGet<ApiResponse<ScheduleDetails>>(`/schedules/${scheduleId}`);
...
export const searchSchedule = async (keyword: string) => {
  try {
    const response = await authPost<ApiResponse<{ searchList: Schedule[] }>>('/schedule/search', {
      keyword,
    });
...
export const createScheduleWithDetails = async (schedule: ScheduleDetails) => {
  try {
    const response = await authPost<ApiResponse<null>>(`/schedules/${schedule.scheduleId}`, {
      scheduleName: schedule.scheduleName,
      startDate: schedule.startDate,
      endDate: schedule.endDate,
...
export const updateSchedule = async (schedule: ScheduleDetails) => {
  try {
    const response = await authPatch<ApiResponse<null>>(`/schedules/${schedule.scheduleId}`, {
      scheduleName: schedule.scheduleName,
      startDate: schedule.startDate,
      endDate: schedule.endDate,
      attendees: schedule.attendees,
